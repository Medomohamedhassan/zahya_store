...
class _GuestCheckoutAddressState extends State<GuestCheckoutAddress> {
  ...
  _onTapOpenAddressDialog() async {
    ...
    if (_allowedCountryCodesForPhone.isNotEmpty) {
      _initialPhoneNumber =
          PhoneNumber(isoCode: _allowedCountryCodesForPhone.first);
    }
  ...
  Future<void> _continueToDeliveryInfo() async {
    ...
    if (guestCheckResponse.result!) {
      ...
      Map<String, dynamic> shippingAddress = {
        ...
        "state_id": _selected_state?.id.toString() ?? "0",
        "city_id": _selected_city!.id.toString(),
        "area_id": _selected_area?.id.toString(),
  ...
  void _onSelectCountry(Country country, StateSetter setModalState) {
    if (_selected_country?.id == country.id) return;
    setModalState(() {
  ...
  void _onSelectState(MyState state, StateSetter setModalState) {
    if (_selected_state?.id == state.id) return;
    setModalState(() {
  ...
  void _onSelectCity(City city, StateSetter setModalState) async {
    if (_selected_city?.id == city.id) return;
    var areaResponse =
  ...
  void _onSelectArea(City area, StateSetter setModalState) {
    if (_selected_area?.id == area.id) return;
    setModalState(() {
      _selected_area = area;
  ...
  Widget _buildCityTypeAheadInDialog(
      bool showStateField, StateSetter setModalState) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Padding(
      ...
      children: [
        ...
        SizedBox(
          ...
          child: TypeAheadField<City>(
            ...
            suggestionsCallback: (name) async {
              if (_selected_city == null) return [];
              var areaResponse = await AddressRepository()
                  .getAriaListByCity(city_id: _selected_city!.id!, name: name);
