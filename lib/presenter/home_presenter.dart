...
class HomePresenter extends ChangeNotifier {
  ...
  int cartCount = 0;
  var randomProductList = [];
  bool isRandomProductInitial = true;
  ...
  fetchRandomProducts() async {
    isRandomProductInitial = true;
    var productResponse = await ProductRepository().getBestSellingProducts();
    if (productResponse.products != null) {
      randomProductList.addAll(productResponse.products!);
      randomProductList.shuffle();
    }
    isRandomProductInitial = false;
    notifyListeners();
  }
  ...
  fetchLocation() async {
    try {
      ...
      if (placemarks.isNotEmpty) {
        ...
        currentLocationText =
            "Deliver to ${locality}${subLocality.isNotEmpty ? " - $subLocality" : ""}";
      } else {
      ...
      currentLocationText = "Error getting location";
      notifyListeners();
    }
  }
